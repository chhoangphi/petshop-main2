...
public class ProductsByItemIDDto extends BaseDao {
	...
	public String SqlProductByTypeOfProduct(String item_id,String type_id) {
		StringBuffer sql=SqlProductByItemID(item_id);
		sql.append("AND tp.id='"+type_id+"' ");
		return sql.toString();
	}
	
	public String SqlProductPagiante(String item_id,int start,int totalPage) {
		StringBuffer sql=SqlProductByItemID(item_id);
		sql.append("LIMIT ");
    ...
    public List<ProductDto> GetDataProductPaginate(String item_id,int start,int totalPage){
    	try {
    	String sql=SqlProductPagiante(item_id,start,totalPage);
    ...
    public List<ProductDto> GetDataProductByTypeOfProduct(String item_id,String type_id){
    	try {
    	String sql=SqlProductByTypeOfProduct(item_id,type_id);
    	System.out.println("SQL Query: "+sql);
		List<ProductDto> list=_JdbcTemplate.query(sql, new MapperProductDto());
		return list;
    	}catch (Exception e) {
